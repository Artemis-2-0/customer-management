...
# Extract the version below <artifactId>customer-management</artifactId>
CURRENT_VERSION=$(awk '
    /<artifactId>customer-management<\/artifactId>/ { found=1 }
    found && /<version>/ {
...
if [[ -z "$CURRENT_VERSION" ]]; then
    echo "Error: Unable to find <version> tag for <artifactId>customer-management</artifactId> in pom.xml"
    exit 1
fi
...
# Update only the version below <artifactId>customer-management</artifactId>
